import aiosqlite
import random
from datetime import date
from typing import Optional
from .db import DB_PATH

# Narxlar (min, max)
SHOP_PRICES = {
    "Common":     (100,   400),
    "Rare":       (400,   1200),
    "Super Rare": (1200,  3500),
    "Epic":       (3500,  9000),
    "Mythic":     (15000, 60000),
    "Legendary":  (80000, 250000),
    "Premium":    (10000, 40000),
    "Exclusive":  (200000, 999999),
}

# Har slot uchun rarity ehtimolligi
SLOT_RARITIES = [
    # Slot 1
    ["Common", "Common", "Common", "Common", "Rare"],
    # Slot 2
    ["Common", "Common", "Rare", "Rare", "Super Rare"],
    # Slot 3
    ["Rare", "Rare", "Super Rare", "Super Rare", "Epic"],
    # Slot 4
    ["Super Rare", "Epic", "Epic", "Mythic"],
    # Slot 5 — surprise: 40C 25R 15SR 10E 6M 3L 1P
    (
        ["Common"] * 40 + ["Rare"] * 25 + ["Super Rare"] * 15 +
        ["Epic"] * 10 + ["Mythic"] * 6 + ["Legendary"] * 3 + ["Premium"] * 1
    ),
]


def _today() -> str:
    return date.today().isoformat()


def _random_price(rarity: str) -> int:
    lo, hi = SHOP_PRICES.get(rarity, (100, 1000))
    return random.randint(lo, hi)


async def get_daily_shop(user_id: int) -> list:
    today = _today()
    async with aiosqlite.connect(DB_PATH) as db:
        db.row_factory = aiosqlite.Row
        cursor = await db.execute(
            """SELECT ds.slot, ds.price, ds.is_sold, ds.waifu_id,
                      w.name, w.anime, w.rarity, w.file_id
               FROM daily_shop ds
               JOIN waifus w ON ds.waifu_id = w.waifu_id
               WHERE ds.user_id=? AND ds.shop_date=?
               ORDER BY ds.slot""",
            (user_id, today)
        )
        rows = await cursor.fetchall()
        return [dict(r) for r in rows]


async def generate_daily_shop(user_id: int) -> list:
    today = _today()
    async with aiosqlite.connect(DB_PATH) as db:
        db.row_factory = aiosqlite.Row

        # Avval bugungi do'konni tozalaymiz
        await db.execute(
            "DELETE FROM daily_shop WHERE user_id=? AND shop_date=?",
            (user_id, today)
        )

        used_ids = set()
        items = []

        for slot_idx, rarity_pool in enumerate(SLOT_RARITIES, start=1):
            rarity = random.choice(rarity_pool)
            # O'sha rarity dan random waifu topamiz
            cursor = await db.execute(
                "SELECT waifu_id, name, anime, rarity, file_id FROM waifus WHERE rarity=? AND is_active=1 ORDER BY RANDOM() LIMIT 10",
                (rarity,)
            )
            candidates = await cursor.fetchall()
            # Bo'sh bo'lsa boshqa raritydan olamiz
            if not candidates:
                cursor = await db.execute(
                    "SELECT waifu_id, name, anime, rarity, file_id FROM waifus WHERE is_active=1 ORDER BY RANDOM() LIMIT 10"
                )
                candidates = await cursor.fetchall()

            waifu = None
            for c in candidates:
                if c["waifu_id"] not in used_ids:
                    waifu = dict(c)
                    used_ids.add(c["waifu_id"])
                    break

            if not waifu:
                continue

            price = _random_price(waifu["rarity"])
            await db.execute(
                """INSERT INTO daily_shop (user_id, shop_date, slot, waifu_id, price, is_sold)
                   VALUES (?, ?, ?, ?, ?, 0)""",
                (user_id, today, slot_idx, waifu["waifu_id"], price)
            )
            items.append({
                "slot": slot_idx,
                "waifu_id": waifu["waifu_id"],
                "name": waifu["name"],
                "anime": waifu["anime"],
                "rarity": waifu["rarity"],
                "file_id": waifu["file_id"],
                "price": price,
                "is_sold": 0,
            })

        await db.commit()
        return items


async def buy_shop_slot(user_id: int, slot: int) -> dict:
    today = _today()
    async with aiosqlite.connect(DB_PATH) as db:
        db.row_factory = aiosqlite.Row
        cursor = await db.execute(
            """SELECT ds.*, w.name, w.anime, w.rarity, w.file_id
               FROM daily_shop ds JOIN waifus w ON ds.waifu_id=w.waifu_id
               WHERE ds.user_id=? AND ds.shop_date=? AND ds.slot=?""",
            (user_id, today, slot)
        )
        row = await cursor.fetchone()
        if not row:
            return {"error": "Slot topilmadi"}
        item = dict(row)
        if item["is_sold"]:
            return {"error": "Bu slot allaqachon sotib olingan"}

        # Foydalanuvchi coini
        cursor2 = await db.execute("SELECT coins FROM users WHERE user_id=?", (user_id,))
        urow = await cursor2.fetchone()
        if not urow or urow["coins"] < item["price"]:
            return {"error": f"Coiningiz yetarli emas. Kerak: {item['price']:,}"}

        await db.execute("UPDATE users SET coins=coins-? WHERE user_id=?", (item["price"], user_id))
        await db.execute(
            "UPDATE daily_shop SET is_sold=1 WHERE user_id=? AND shop_date=? AND slot=?",
            (user_id, today, slot)
        )
        await db.execute(
            "INSERT INTO collections (user_id, waifu_id) VALUES (?,?)",
            (user_id, item["waifu_id"])
        )
        await db.execute("UPDATE users SET total_caught=total_caught+1 WHERE user_id=?", (user_id,))
        await db.commit()
        return {"ok": True, "item": item}


async def get_or_create_shop(user_id: int) -> list:
    items = await get_daily_shop(user_id)
    if not items:
        items = await generate_daily_shop(user_id)
    return items
